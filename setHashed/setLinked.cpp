...
// setLinked& setLinked::operator=(setLinked&& source){
//         if (&source != this)
//     {
//         cell* temp;
//         cell* next;
...
//     return *this;
// }
bool setLinked::isEqual(setLinked& set){
    if(&set != this){
        int amountOfEqualLists;
        cell* thisTemp = this->listSet.First();
        cell* setTemp = set.listSet.First();
        while(thisTemp != nullptr && setTemp != nullptr){
            if(thisTemp->element != setTemp->element){
                return false;
            }
        }
        return true;
